<?php
    ...
    $mysqli->query("DELETE FROM `login_attempts_ip` WHERE `last_request_time` < " . ($current_time - 10));
    ...
    $id = -1;
    if ($user) {
        if ($user['locked_until'] != null && strtotime($user['locked_until']) > $current_time) {
            echo md5(md5(-1));
            exit();
        }
        ...
        if ($user['password'] === $password) {
            ...
            if ($attempts >= 5) {
                $locked_until = date("Y-m-d H:i:s", $current_time + 300);
